...
export function RoomVoicePanel({
  ...
  return (
    <section
      className={`rounded-2xl border border-emerald-200/20 bg-emerald-950/20 ${
        compact ? "max-w-60 flex-1 px-3 py-1.5" : "px-3 py-2"
      } ${className ?? ""}`}
    >
      <div className={`flex min-w-0 items-center gap-2 ${compact ? "" : "whitespace-nowrap"}`}>
        <p className="shrink-0 text-[11px] font-semibold uppercase tracking-[0.14em] text-emerald-200/75">
          Room Voice
        </p>
        <span className="truncate text-xs font-semibold text-emerald-100/90">
          {statusLabel}
        </span>
        ...
        <div className="ml-auto flex items-center gap-2">
          <button
            ...
            className={`cursor-pointer grid place-items-center rounded-lg border border-emerald-100/30 bg-emerald-500/10 text-emerald-100 transition hover:bg-emerald-500/20 disabled:cursor-not-allowed disabled:opacity-60 ${
          ...
          <button
            ...
            className={`cursor-pointer grid place-items-center rounded-lg border border-sky-100/30 bg-sky-500/10 text-sky-100 transition hover:bg-sky-500/20 disabled:cursor-not-allowed disabled:opacity-60 ${
              compact ? "h-8 w-8" : "h-9 w-9"
            }`}
          >
            {isSpeakerMuted ? (
